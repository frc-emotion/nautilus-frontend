...
export const NotificationsProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    const addUpdate = useCallback(async (updatefr: string) => {
        if (!user?._id) {
            console.error(`${DEBUG_PREFIX} Cannot add update: User not authenticated`);
            return;
        }
        const request: QueuedRequest = {
    ...
    const updateUpdate = useCallback(async (updatefr: string, idfr: number) => {
        if (!user?._id) {
            console.error(`${DEBUG_PREFIX} Cannot update update: User not authenticated`);
            return;
        }
        const request: QueuedRequest = {
    ...
    const removeUpdate = useCallback(async (id: number) => {
        if (!user?._id) {
            console.error(`${DEBUG_PREFIX} Cannot remove update: User not authenticated`);
            return;
        }
        const request: QueuedRequest = {
